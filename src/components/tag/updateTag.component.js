...
const { updateTag } = require("../../services/tag.service.js")
...
const UpdateTag = (props) => {
  ...
  const tagupdate = JSON.parse(localStorage.getItem("tagupdate"))

  const [name, setName] = useState(tagupdate.name);
  const [infor, setInfor] = useState(tagupdate.infor);
  ...
  const handleUpdate = (e) => {
    ...
    if (checkBtn.current.context._errors.length === 0) {
      const id= props.match.params.idtag;
      updateTag(id, name, infor)
      .then(()=>{
        props.history.push("/board/"+tagupdate.idboard);
    ...
    <div className="col-md-12">
      ...
      <div className="card card-container">
        ...
        <Form onSubmit={handleUpdate} ref={form}>
          ...
          <div className="form-group">
            <button className="btn btn-primary btn-block">Update Tag</button>
          </div>
          {message && (
            <div className="form-group">
              <div className={"alert alert-danger"} role="alert">
